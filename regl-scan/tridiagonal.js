...
const prefixSum = require('./')(regl, {
  ...
  reduce: [
    `vec4 reduce(vec4 prefix1, vec4 prefix2, vec4 sum1, vec4 sum2) {
      return vec4(
        sum1.x * prefix1.x + sum1.y * prefix2.x,
        sum1.x * prefix1.y + sum1.y * prefix2.y,
        sum1.x * prefix1.z + sum1.y * prefix2.z, 0);
    }`,
    `vec4 reduce(vec4 prefix1, vec4 prefix2, vec4 sum1, vec4 sum2) {
      return vec4(
        sum2.x * prefix1.x + sum2.y * prefix2.x,
        sum2.x * prefix1.y + sum2.y * prefix2.y,
        sum2.x * prefix1.z + sum2.y * prefix2.z, 0);
    }`
  ]
});
